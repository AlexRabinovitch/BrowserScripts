...
function processNonTitleItems(itemFilter, node, selector, itemName)
{
    if(itemFilter)
    {
        ...
        //console.log(itemName + 's found: ' + items.length);
...
function fillFilters()
{
    ...
    var commonExcludedSellers = ['gtohall', 'bhall0415', ' alamo3636', 'laparkamania', 'softball-1baseball-2', 'jonsmemorabiliamart64', 'picker_picker', 'oldfartcollections', 'annfrei_12', 'luckynickels', 'griff2299', 'sstoys2', 'kvik_567', 'mirthunder', 'brcli_25', 'kerrlindle0', 'crash29', 'janerose2014', 'softviking', 'tbt', 'davenportpm1', 'leopardlover48', 'www.telecoin.bizland.com', 'infernal_devices', '2rosesathome', 'j_byers_24', 'the\\*village\\*idiot', 'vintagestg', 'littletoymaker', 'realbricks', 'goatsonparade17023', 'alysunwonderland', 'tin-toyman', 'jhsmith400', 'cumberlandborn', 'mediawavepress', 'medienvertriebundverlag', 'hustlebustleandrustle', 'mee_1767', 'kavcha_54', 'babasehgal', 'smartpickhub', 'allenwoodtrading'];
    ...
    var marxExcludedTerms = ['fortune', 'sleeping beauty', '\\bnot? Marx\\b', 'Marx-U', 'marx style', 'Marx.?\\?', 'marx( toys)? \\(\\?\\)', 'good with marx', 'replicants', 'MPC', 'tim.?mee', 'Linemar', 'Karl Marx', 'Marx, Karl', 'Engels', 'Spencer', 'Barzso', 'Schaffner', 'Richard Marx', 'Marx brothers', 'Maddox', 'Garret', 'rides again', 'Marx similar', 'Herbert', 'Thunderbolt', 'jean h(oe|ö)fler','^(?=.*(?:\\btin\\b))(?!.*(?:rin.*tin.*tin))', '10\'\'', '9"', '12"'];
    ...
    var excludeTermsElastolin = ['\\bgerman(s)?\\b', 'Prussian', 'wehrmacht', 'Luftwaffe', 'Preu(ß|ss)', 'soldat allemand', 'ähnlich Elastolin', 'auch zu Elastolin', 'elastolin compatible', 'elastolin\\?', '(suitable|matching|fits|compatible) (for|to|type|wie|with) Elastolin', '\\bno Elastolin', 'w\\/Elastolin', 'not elastolin', 'non elastolin', '\\b(zu|for|to) Elastolin', '^elastolin$', 'bundeswehr', 'Musikkorps', 'musiker', 'Musikkapelle', 'musician', 'waffen', '\\bSS\\b', '\\bScott', '\\bnazi\\b', 'general', 'field marshal', 'lansquenet', 'landsknecht', 'cannon', 'howitzer', '\\bwwi', 'Feuerlöschboot', 'fusser'];
  	var excludeCardsRelated = ['digimon', 'benalish', 'wanderbrine', 'TCG', 'vtes', 'Bofuri', 'midnight hunt', 'Weiss Schwarz', 'rathi', 'yu.?gi.?oh', 'mirkwood', 'alena', 'gavony', 'avenant', 'strawberry', 'dauthi', 'Bloomburrow', 'mouse', 'geist', 'Ballynock', 'Dominaria'];

    var commonExcludeCountries = ['China'];

    var filter = new Object();
    filter.searchName = "1"; // 4, 5, 6 inch cowboys and indians
    filter.mustTerms = inch456;
    filter.excludeTerms = ['Chap Mei', 'giraffe', 'clicker', 'donald', 'baby', 'boot', 'subway', 'disney', 'GI Joe', 'my little pony', 'menko', 'cowboy hat', 'Logan', 'Orton', 'Chip Mei', 'Totsy', 'rocking', 'slug', 'NFL', 'Owens', 'chess', 'Posable', 'elephant', 'blanket', 'pocahontas', 'Wolverine', 'McFarlane', 'Skyforce', 'Prescott', 'penguin', 'potato', 'paper mache', 'photo'];
...
//    filter.excludeItemIDs = [154246251005];
    ...
    filter = new Object();
    filter.searchName = "2"; // 4, 5, 6 inch indians
    filter.mustTerms = inch456;
    ...
    filter = new Object();
    filter.searchName = "3"; // mounted cowboys and indians
    filter.excludeTerms = ['Tradition', 'Trophy Miniatures', 'Traditional Models', 'nursery', 'wooden', 'halloween', 'mvp', 'Barclay', 'Manoil', 'Geobra', 'outfit', 'Safari', 'Schleich', 'paddle', 'elephant', 'token', 'Indian army', 'Woody', 'jacket', 'drum', 'child', 'lionel', 'horseshoe', 'folk art', 'mutiny', 'holster', 'board game', '\\btin\\b', 'Star Ace', 'Hopalong', 'country', 'Atkins', 'Bravestar', 'flock', 'legends', 'stencil', 'ducky', 'Peppa pig', 'cowboy hat', 'cowboy boot', 'domino', 'mechanical', 'card', 'yo.?yo', 'W Britain', 'britain\'?s?', 'purse', 'wallet', 'Roll Call', 'velvet', 'blind', 'clip.*?clop', 'stick', 'space', 'Hoefler', 'buckle', 'scarf', 'ponywars', 'frogz', 'roadrider', 'schylling', 'chap mei', 'giant', 'boot hill', 'GMT', 'forged', 'First Legion', 'handcraft', 'canteen', 'Kingcast', 'Frontline Figures', 'douglas', 'sikh', 'minifig', '\\bthreeA|3A\\b', '\\braj\\b', 'beanbag', 'gabriel', 'new.?ray', 'road.?rider', 'cuddle', 'marionette', 'christma', 'littlest', 'tick.?tock', 'rocking', 'farm world', 'monster', 'pendulum', 'weeble', 'michel', 'duck', 'ganz', 'inflatable', 'bucking bulls', 'Rudolph', 'Merten', 'resin', 'ro(bo)?blox', '\\bmtb\\b', 'keystone', '\\belmo', 'delhi', 'durbar', 'Skinner', 'Bengal', 'lancer', 'clothes', 'bouncy', 'curtains', 'lancer', '\\b(4|8|16|21).?pcs\\b', 'Judy Larson', 'skull', 'unimax', 'krolyn', 'RCOMG', 'girls', 'micro', 'pillow', 'nosco', '\\bPapo\\b', '\\bMOJO\\b', 'gripping', 'roper', 'Springfield', 'windshield', 'camel', 'ice.?cream', '48 pcs'];
    filter.excludeTerms = filter.excludeTerms.concat(commonExcludedTerms);
    filter.excludeTerms = filter.excludeTerms.concat(commonIrrelevantSizes);
    filter.excludeSellers = ['denyakim', 'ourfinds', 'northstate', 'labellesassy', 'happinessfund', 'grg.store', 'yannis1960grvtg', 'mercator_trading', 'rustybargainwarehouse', 'everyday_nest'];
    filter.excludeSellers = filter.excludeSellers.concat(commonExcludedSellers);
    ...
    filter = new Object();
    filter.searchName = "4"; // frontiersmen
    filter.excludeTerms = ['\\breel', 'Bushido', 'Action.?Man', 'Action.?Team', '\\blead\\b', 'coin', 'Glass', 'teddy.?bear', 'penguin', 'Corvus', 'Loggers', 'Wargame', 'board', 'Malifaux', 'Ninja', 'Time', 'Bombshell', '\bdnd\b', 'ad&d', 'warhammer', 'Bachmann', 'Citadel', 'Skylander', 'Dakin', 'Pok(é|e)mon', 'speed', 'lost world', 'pillow', 'wyrd', 'wallet', 'gloves', 'Mordenkainen', 'Mandalorian', 'duluth', 'demigod', 'resin', 'Monsters', 'hell.?dorado', 'baseball', 'belt', 'badge', 'paw patrol', 'ruby', 'gator', 'Buddy.?L', 'alien', 'Artisan', 'Bushiroad', 'Greyjoy', 'pocket knife', 'storm.?trooper', '\\bgame\\b', 'hand.?made', '\\bdnd\\b', 'robot', 'outfit', 'Figma', 'truck', '\\bhat\\b', 'Jurassic', 'dragon trapper', 'nendoroid', 'dbd', 'bear', '\\bdecal(s)?\\b', 'locomotive', 'gnome', 'dirt.?trapper', 'privateer', 'MaxMini', 'gnarl', 'rpg', 'croak', 'soul.?trapper', 'frostgrave', 'dungeons', 'dead( )?by( )?daylight', 'timpo', 'helmet', 'flyer', 'freefolk', 'battalion', 'Kenner', 'Bachman', 'Goofy', 'Lilo.*Stitch', 'lone ranger', 'kentucky long .*rifle', 'icewind', 'beanie', 'trapper wolf', '\\bPSL\\b', 'fallout', 'wizkids', 'demon'];
    filter.excludeTerms = filter.excludeTerms.concat(commonExcludedTerms);
    filter.excludeTerms = filter.excludeTerms.concat(commonIrrelevantSizes);
    ...
    filter = new Object();
    filter.searchName = "5"; // Marx knights
    ...
    filter = new Object();
    filter.searchName = "7"; // Marx cowboys
    ...
    filter = new Object();
    filter.searchName = "9"; // Auburn //worldwide auburn (cowboy,cowboys,indian,indians,frontiersman,frontiersmen,frontier,pioneer,pioneers,horse,horses) -("western models",shoe,shoes,horseshoe,police,farm,"rubber axe","id-9")
    filter.excludeTerms = ['Kusan', 'hammer', 'motorcycle', 'hatchet'];
    _filters[filter.searchName] = filter;

    filter = new Object();
    filter.searchName = "10"; // Elastolin
    filter.excludeTerms = ['ground', 'composée', 'composit','compo', '\\bmasse?\\b', 'massesoldat', 'masse.?figur', '\\bpaste\\b', '\\bpasta\\b', 'british', 'French', '7th', 'Civil war', 'Zusammensetzung', 'Clairet', 'massebäume', 'elfer raus', 'turk', 'ww1', 'revolution', 'pre war', 'Africa', '1930s', '\\b1940\\b', 'trooper', '\\bband\\b', 'janestkzi', 'railway', 'reamsa', 'jecsan', '\\bfaro\\b', 'animal', 'giraffe', 'mercenary?(ies)?', 'doughboy', '\\bdeer', '\\bboa\\b', 'tiger', '\\blion', '\\bzoo\\b', '\\bbear\\b', 'monkey', 'ostrich', 'rhino', 'wolf', 'panther', 'zebra', 'prewar', 'Steckfigur', 'connector', 'socket', 'regiment', 'grenadier', 'lafredo', 'M[aä]rklin', '\\blead\\b', 'ADVERTISING Brands', 'diedhoff', 'quiralu', 'blechräder', 'air france', 'advertising', 'plug', 'nativity', 'ww2', 'wwii', 'Weltkrieg', 'world war', 'mussolini', 'militaria', '\\bWK\\b', 'Kellermann', 'Gama', 'sailor', 'kriegsmarine', '\\btin\\b', '\\btipp?co', 'goering', 'german politician', 'Jean H(ö|o)fler', '\\b19[01234]\\d', '44\\d{2}', '4\\d{4}', '\\b549(3|4)\\d{1}', '\\b1(4|6)\\d{3}', '\\b541\\d{2}', '\\b780\\d{1}\\b', '\\b73(3|4|5)\\d{1}\\b', '\\b5(6|7|8)\\d{3}', '5.6cm', '1\\:72', 'CM 4', '\\b1 5\\/8', '\\b1 3\\/4', '1/50', '1.5 Inch', '46.?mm', '38.?mm', '19.?mm', '10.5.?cm'];
    filter.excludeTerms = filter.excludeTerms.concat(commonExcludedTerms);
    filter.excludeTerms = filter.excludeTerms.concat(excludeTermsElastolin);
    filter.excludeSellers = ['ship0v', 'frichmuthboy', 'el-siluro', 'jskins72', 'oz75', 'brams741', 'dancas3012', 'michaelkinhart', 'karoly558', 'lobster75459', ' fb78', 'vintage_villages', 'petitssoldatsmania', 'sopilo', 'antik_kro', 'fraley35'];
    filter.complexTerms = [];
    filter.complexTerms.push(buildComplexTermsFilter(['soldier', 'soldiers', 'soldat', 'military'], ['Norman', 'Roman', 'cowboy', 'indian', 'native', 'medieval', 'middle', 'castle', 'knight', 'ritter', 'Karl May', 'catapult', 'viking', 'gaul', 'toy']));
    filter.complexTerms.push(buildComplexTermsFilter(['officer'], ['Roman']));
    _filters[filter.searchName] = filter;

    filter = new Object();
    filter.searchName = "11"; // Empire //worldwide //empire (indian,indians,pioneer,pioneers,frontiersmen,frontiersman,1776,frontier) -(magazine,puzzle,rpg,star,citadel,chess,army,hachette,lead,king,north,25mm) -(geronimo,defiance,command,metal,carolina,sikh,15mm,"id-11")
    filter.excludeTerms = ['Hachette-marshals', 'Spirit of the Empire', 'Sons of the Empire', 'redcoat', 'empire state', 'Sons of Empire', 'Warhammer', 'fire (dept )?chief', 'Roman empire', 'end of empire', 'british'];
    filter.excludeTerms = filter.excludeTerms.concat(commonExcludedTerms);
    filter.excludeSellers = ['oraff_tm', 'libro_di_faccia'];
    _filters[filter.searchName] = filter;

    filter = new Object();
    filter.searchName = "12"; // Ideal knights // ideal (knight,knights) -(slot,batman,galahad,ingrid,"star team",darkness,tcr,trap,cutoff,"id-12")
    ...
    filter = new Object();
    filter.searchName = "13"; // Ideal pirates
    ...
    filter = new Object();
    filter.searchName = "18"; // Marx frontiersmen
    ...
    filter = new Object();
    filter.searchName = "19"; // Marx horses
    ...
    filter = new Object();
    filter.searchName = "20"; // Marx Indians
    filter.excludeTerms = ['\\bcar\\b', 'target', 'skate', 'Big Jim', 'Wildflower', 'police', 'whirling', 'Brave Eagle', 'fire (dept )?chief', 'lone ranger'];
    filter.excludeTerms = filter.excludeTerms.concat(commonExcludedTerms);
    filter.excludeTerms = filter.excludeTerms.concat(marxExcludedTerms);
    filter.excludeTerms = filter.excludeTerms.concat(commonIrrelevantSizes);
    ...
    filter = new Object();
    filter.searchName = "22"; // Marx pirates // worldwide // marx (pirate,pirates) -("ramp walker","johnny west","id-22")
    ...
    filter = new Object();
    filter.searchName = "23"; // Marx saddles
    ...
    filter = new Object();
    filter.searchName = "24"; // Marx vikings // worldwide // marx (viking,vikings) -(odin,sir,"johnny west","karl marx",cxr,"1/6","12"") -(brave,"erik the viking","eric the viking","id-24"))
    ...
    // Dulcop knights // worldwide // dulcop (knight,knights,cavaliere,cavalieri,crusade,crusades,crociate,medieval,medievali) -(supercharly) // maryjack2012,franinca1
    // Gunsmoke // worldwide // marx (gunsmoke,matt,dillon,doc,kitty,chester) -(president,presidents,"chester arthur",linemar,"johnny west",kat,mug,gould,locomotive,"wind-up",cat,"ready gang",disneykins,major,flintstones,"matt mason",cabin,flintstone,holiday,holliday,wannatoy,roberts,compatible,airport,dale,"ramp walker",1/6) // anh60,upstate-auctions,cityscape-artifacts,littletoymaker,joholle-5,magbre30,running-down-a-deal,gtohall,anh60,upstate-auctions,cityscape-artifacts,littletoymaker,joholle-5,magbre30,running-down-a-deal,gtohall,starline2000,tullorderpickers
...
function checkURL()
{
    ...
    {
        _searchID = res[0].replace(/%22/g, "").replace("id", "").replace("-", "");
