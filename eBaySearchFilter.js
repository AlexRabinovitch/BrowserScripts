...
function fillFilters()
{
    ...
    var inch456Exclude = ['crescent', 'payton', 'duck', 'bend', 'Squishmallows', 'drum', 'bendable', '\\bpee', 'doctor who', 'rescue', 'dime novel', '1 1/4"', '1.5"', '1 3/4"', '2 1/4', '2.5”', '2 3/4"', '2-1/4', '2 1/4"', '2.5”', '2-3/4"', '2 3/4', '2.75"', '1:12', '8 3/4"', '12.5"'];

    var commonExcludedSellers = ['gtohall', 'bhall0415', ' alamo3636', 'laparkamania', 'softball-1baseball-2', 'jonsmemorabiliamart64', 'picker_picker', 'oldfartcollections', 'annfrei_12', 'luckynickels', 'griff2299', 'sstoys2', 'kvik_567', 'mirthunder', 'brcli_25', 'kerrlindle0', 'crash29', 'janerose2014', 'softviking', 'pezdudewelch', 'tbt', 'davenportpm1', 'leopardlover48', 'www.telecoin.bizland.com', 'infernal_devices', '2rosesathome', 'j_byers_24', 'the\\*village\\*idiot', 'vintagestg', 'littletoymaker', 'realbricks', 'goatsonparade17023', 'alysunwonderland', 'tin-toyman', 'jhsmith400', 'cumberlandborn'];
    var commonExcludedTerms = ['(Joh?h?n?ny|Jane|Jamie|Jimmy|Josie|Jay) West', 'Best of (the )?West', 'Ready Gang', 'Legends of (the )?West', 'cherokee', 'cap.?gun', '\\bw(ind)?.?up\\b', 'ring.?hand', 'pin.?ball', 'puzzle', 'Barclay', 'Manoil', 'play.?mobil', 'friction', 'ajax', '\\bdolls?\\b', 'ram.?p.?walk', 'Bravo', '\\bH(O|0)\\b', 'HO scale', 'Hasbro', 'marble', 'swirl', 'jabo', 'celluloid', 'harmonica', 'plush', 'navwar', 'giant brand', 'Tonka', 'POST.?CARD', 'swoppet?d?', 'swivel', 'moo mesa', 'marchon', 'little.?people', 'gbpv', 'Popeye', 'Fisc?her.?Price', 'Bonanza', 'clock.?work', '\\bpull.?toy\\b', '\\btin.?plate\\b'];
    var marxExcludedTerms = ['fortune', 'Gabriel', 'sleeping beauty', '\\bno Marx\\b', 'nos rare Marx$', ', Marx$', 'Marx!!!$', 'Marx-U', 'marx style', 'Marx.?\\?', 'marx( toys)? \\(\\?\\)', 'good with marx', 'replicants', 'MPC', 'tim.?mee', 'Linemar', 'play.?people', 'Karl Marx', 'Spencer', 'Barzso'];

    var excludeSellersHorsesSaddles = ['apeman28', 'recordsiam7', 'rugbychick12'];
    var excludeTermssHorsesSaddles = ['Pa?o?ncho', 'Marvel', 'Valou?r', 'movable', 'joint', 'articulated', 'Valiant', 'Breyer'];

    var filter = new Object();
    filter.searchName = "id-1"; // 4, 5, 6 inch cowboys and indians
    filter.mustTerms = inch456;
    filter.excludeTerms = ['Chap Mei', 'Pail', 'Sesame', 'giraffe', 'clicker', 'donald', 'baby', 'boot', 'subway', 'disney', 'Playskool', 'GI Joe', 'my little pony', 'menko', 'cowboy hat', 'Logan', 'Orton', 'Chip Mei', 'Totsy', 'rocking', 'slug', 'NFL', 'chess', 'Posable', 'elephant', 'blanket', 'pocahontas', 'Wolverine', 'McFarlane', 'Skyforce'];
    ...
    filter.excludeTerms = ['lithographed', 'crazy', 'target', 'Buck Rogers', 'lledo', 'cap gun', 'linemar', 'gijoe', 'reamsa', 'jecsan', 'ride.on', 'johnnie', '7.5"', ' 12”', '1/72'];
    filter.excludeTerms = filter.excludeTerms.concat(commonExcludedTerms);
    filter.excludeTerms = filter.excludeTerms.concat(marxExcludedTerms);
    ...
    filter.excludeTerms = ['ground', 'british', '\\bgerman\\b', 'composée', 'composit','compo', '\\bmasse\\b', 'French', '7th', 'Allemand', 'Civil war', 'Zusammensetzung', 'Clairet', 'massebäume', '^elastolin$', 'elfer raus', 'turk', 'ww1', 'revolution', 'pre war', 'not elastolin', 'non elastolin', 'Africa', '1930s', 'trooper', '\\bcow\\b', '\\bband\\b', 'janestkzi', 'prussian', 'railway', 'reamsa', 'jecsan', 'lansquenet', 'landsknecht', '\\bfaro\\b', 'animal', 'military', 'giraffe', 'mercenary?(ies)?', 'doughboy', '\\bdeer', '\\bboa\\b', 'tiger', '\\blion', '\\bzoo\\b', '\\bbear\\b', 'monkey', 'ostrich', 'rhino', 'wolf', 'panther', 'zebra', 'type elastolin', '1930', 'CM 4', '1 5/8', '1/50', '1.5 Inch'];
    //filter.excludeTerms = ['1930'];
    filter.excludeTerms = filter.excludeTerms.concat(commonExcludedTerms);
    filter.excludeSellers = ['ship0v', 'frichmuthboy', 'el-siluro', 'jskins72', 'oz75', 'brams741', 'dancas3012', 'michaelkinhart', 'karoly558', 'lobster75459', ' fb78'];
    filter.complexTerms = [];
    filter.complexTerms.push(buildComplexTermsFilter(['soldier'], ['Norman', 'Roman', 'cowboy', 'indian', 'native', 'medieval', 'middle', 'castle', 'knight', 'ritter', 'Karl May', 'catapult']));
    _filters[filter.searchName] = filter;

    filter = new Object();
    filter.searchName = "id-11"; // Empire
    filter.excludeTerms = ['Hachette-marshals', 'Spirit of the Empire', 'Sons of the Empire', 'redcoat', 'empire state', 'Sons of Empire', 'Warhammer', 'fire chief', 'Roman empire', 'end of empire', 'pullman', 'playmobil', 'british'];
    filter.excludeTerms = filter.excludeTerms.concat(commonExcludedTerms);
    ...
    filter.excludeTerms = ['lithographed', '\\bcar\\b', 'target', 'tebur-8', 'skate', 'Big Jim', 'Wildflower', 'police', 'whirling', 'Brave Eagle', '12 inch', '12”'];
    filter.excludeTerms = filter.excludeTerms.concat(commonExcludedTerms);
    filter.excludeTerms = filter.excludeTerms.concat(marxExcludedTerms);
    ...
    filter.excludeTerms = ['sailor queen', 'Goldberg'];
    filter.excludeTerms = filter.excludeTerms.concat(commonExcludedTerms);
    filter.excludeTerms = filter.excludeTerms.concat(marxExcludedTerms);
    ...
    filter.excludeTerms = ['Custer', 'MCMLXVI', 'howdah', '\\bReinhard?t?\\b', '\\blaig\\b', '9”', '9"', '8 inch'];
    filter.excludeTerms = filter.excludeTerms.concat(commonExcludedTerms);
    filter.excludeTerms = filter.excludeTerms.concat(marxExcludedTerms);
    filter.excludeTerms = filter.excludeTerms.concat(excludeTermssHorsesSaddles);
